...
The information came from the columns of the original data frame. R does not does not understand that these character strings represent years, thus its most likely that its a character variable. The variable should instead be numeric.
...
The scale on the x-axis needs to change, along with the axis titles, the plot title as well as the subtitle and caption. The line colors also need to change to their corresponding colors, while the legend title also needs to change to match the goal plot.
...
"AB2" = "lightgoldenrod3")
  ...
  labs(
    ...
    caption = "Source: Office of the University Registrar/nhttps://registrar.duke.edu/registration/enrollment-statistics"
  ) +
  theme_minimal()
